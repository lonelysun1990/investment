...
test("computeDerived reports no capitalRaiseMismatch when implied ownership agrees with stated ownershipPct", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "build-dashboard-test-"));
  const legacyPath = path.join(dir, "legacy.json");
  const mcneilPath = path.join(dir, "mcneil.json");
  const distributionsPath = path.join(dir, "distributions.json");
  ...
  await writeFile(mcneilPath, JSON.stringify({}));
  await writeFile(distributionsPath, JSON.stringify({
    legacy: [{ date: "2026-Q2", myDistribution: 1000, totalDistribution: 4000 }],
  }));
  await writeFile(capitalPath, JSON.stringify({
    legacy: { larryInvestment: 50000, totalRaise: 200000 },
  }));
  ...
  const imported = await import(`file://${outputPath}`);
  assert.equal(imported.DERIVED.legacy.ownershipPct, 25);
  assert.deepEqual(imported.DERIVED.legacy.impliedOwnershipByQuarter, [
    { date: "2026-Q2", impliedOwnershipPct: 25 },
  ]);
  assert.equal(imported.DERIVED.legacy.capitalRaiseMismatch, false);

  await rm(dir, { recursive: true });
});

test("computeDerived flags capitalRaiseMismatch when a quarter's implied ownership disagrees with stated ownershipPct (McNeil scenario)", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "build-dashboard-test-"));
  const legacyPath = path.join(dir, "legacy.json");
  const mcneilPath = path.join(dir, "mcneil.json");
  const distributionsPath = path.join(dir, "distributions.json");
  ...
  await writeFile(distributionsPath, JSON.stringify({
    mcneil: [
      { date: "2025-Q1", myDistribution: 570.37, totalDistribution: null },
  ...
  await writeFile(capitalPath, JSON.stringify({
    mcneil: { larryInvestment: 50000, totalRaise: 1500000 },
  }));
  ...
  const imported = await import(`file://${outputPath}`);
  assert.equal(imported.DERIVED.mcneil.ownershipPct, 3.33);
  assert.deepEqual(imported.DERIVED.mcneil.impliedOwnershipByQuarter, [
    { date: "2026-Q1", impliedOwnershipPct: 0.54 },
    { date: "2026-Q2", impliedOwnershipPct: 2.59 },
  ]);
  assert.equal(imported.DERIVED.mcneil.capitalRaiseMismatch, true);

  await rm(dir, { recursive: true });
});

test("computeDerived leaves impliedOwnershipByQuarter empty and capitalRaiseMismatch false when every totalDistribution is null", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "build-dashboard-test-"));
  const legacyPath = path.join(dir, "legacy.json");
  const mcneilPath = path.join(dir, "mcneil.json");
  const distributionsPath = path.join(dir, "distributions.json");
  ...
  await writeFile(capitalPath, JSON.stringify({
    legacy: { larryInvestment: 50000, totalRaise: 1100000 },
  }));
