...
**Relationship to the existing `rentRoll` occupancy source — full replacement**: `extractMcneilBatch` currently attaches `occupancyPct` from a rent-roll attachment (PDF/XLSX) when one exists for a month, matched by the rent roll's own `asOfDate`. This was confirmed to be the *root cause* of the user's original complaint: rentRoll is the only occupancy source in the codebase today, and it is extremely sparse (in the real committed data it ever covered at most 3 of 22 months — 2025-09, 2025-12, 2026-06 — and the current raw archive only still has the attachment for one of those). The dashboard only ever reads `record.occupancyPct`, never the `record.rentRoll` object itself, so nothing downstream depends on that object's presence. Per explicit user decision: **drop rentRoll from occupancy entirely.** `occupancyPct` for every month, including 2026-06, comes exclusively from the new 3-source priority pipeline (direct statement > vacant-unit narrative > chart). This removes the `rentRolls`/`matchingRentRoll` merge branch from `extractMcneilBatch` and the `record.rentRoll` attachment; the existing test asserting `record.rentRoll` presence (`extract-mcneil.test.mjs:232`) must be deleted or rewritten to match the new behavior, along with any other rentRoll-occupancy-specific tests. Rent-roll files themselves are still downloaded/archived as before (they may carry other per-unit data outside this feature's scope) — only their role as an occupancy source is removed.
