...
- "Total income", "NOI", and "Net income" rows get a new CSS class (e.g. `.row-highlight`) giving thicker top and bottom borders than the table's regular thin gray row lines. The underlying distinction: every row in this table is either a **status** row (a cumulative result — the outcome of adding/subtracting the rows above it: Total income, NOI, Net income) or an **operation** row (a value that gets added or subtracted to move from one status to the next: Rental income, Other income, Total operating expense, Non-operating expense). The thicker border is a subtotal/grand-total rule, the standard accounting-statement convention for marking "this is a sum of what's above," distinguishing results from the inputs that produced them.

## 4. Waterfall chart removed entirely

The "Revenue → NOI → Net income" waterfall chart (`renderMonthlyWaterfallChart`, its `<canvas id="waterfall-${dealSlug}">` card, and its call site in `renderDealView`) is removed outright — every number it plots is already in the ledger table immediately above it, and the ledger table's new highlighting (§3) already marks the same three checkpoints. `dashboard/data.js` and the underlying `nonOperatingExpense` field don't need chart-specific handling as a result.
