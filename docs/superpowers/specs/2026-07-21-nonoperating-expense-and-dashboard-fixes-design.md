...
- "Total income", "NOI", and "Net income" rows get a new CSS class (e.g. `.row-highlight`) giving thicker top and bottom borders than the table's regular thin gray row lines. The underlying distinction: every row in this table is either a **status** row (a cumulative result — the outcome of adding/subtracting the rows above it: Total income, NOI, Net income) or an **operation** row (a value that gets added or subtracted to move from one status to the next: Rental income, Other income, Total operating expense, Non-operating expense). The three highlighted rows are exactly the three waterfall checkpoints already named in the "Revenue → NOI → Net income" chart title above this table — the thicker border is a subtotal/grand-total rule, the standard accounting-statement convention for marking "this is a sum of what's above," distinguishing results from the inputs that produced them.
